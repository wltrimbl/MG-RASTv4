(function () {
	...
	if (md_file_opts.length) {
	    ...
	    md_file_opts = '<p>You do not have any metadata files available. Metadata can be uploaded on the <a href="?mgpage=upload">upload page</a>.</p><p>If you have uploaded metadata and it is not displayed here, it might be invalid. Click on the metadata file in your inbox on the <a href="?mgpage=upload">upload page</a> to receive more information.</p><p>Submission of multiple files, sharing of data or data publication require metadata. You can use <a href="https://github.com/MG-RAST/MG-RASTv4/raw/master/data/MGRAST_MetaData_template_current.xlsx">this Excel template</a> and/or <a href="?mgpage=metazen2" target="_blank">the MetaZen tool</a> to fill out the metadata spreadsheet for a study.</p><p>More information about metadata can be found in the <a href="https://help.mg-rast.org/user_manual.html#metadata" target=_blank>MG-RAST manual, section 2.7</a>.</p>';
	}
	...
	for (var i=0; i<widget.inboxData.length; i++) {
	    if (widget.inboxData[i].hasOwnProperty('data_type') && widget.inboxData[i].data_type == 'sequence') {
		    if (parseInt(widget.inboxData[i].stats_info.length_max) > 500000) {
		        seq_file_opts.push("<option disabled style='color: red;' title='maximum sequence length exceeded ("+widget.inboxData[i].stats_info.length_max+", max is 500,000 bp)'>"+widget.inboxData[i].filename+"</option>");
		    ...
		    } else if (parseInt(widget.inboxData[i].stats_info.sequence_count) < 100) {
                seq_file_opts.push("<option disabled style='color: red;' title='below minimum sequence count ("+widget.inboxData[i].stats_info.sequence_count+", minimum is 100)'>"+widget.inboxData[i].filename+"</option>");
		    } else if (parseInt(widget.inboxData[i].stats_info.bp_count) < (1024 * 1024)) {
                seq_file_opts.push("<option disabled style='color: red;' title='below minimum base pair count ("+widget.inboxData[i].stats_info.bp_count+", minimum is 1,000,000 bp)'>"+widget.inboxData[i].filename+"</option>");
            } else {
	...
	var options = { "fastq": [ { "name": "assembled",
					 ...
					 "class": "checkbox",
					 "checked": false,
					 "text": 'Select this option if your input sequence file(s) contain assembled data and include the coverage information within each sequence header as described <a target="blank" href="https://help.mg-rast.org/user_manual.html">here</a>.' } ] },
				   { "name": "dereplication",
				     "fields": [
					 { "id": "dereplication",
					   "name": "dereplication",
			...
			"fasta":  [ { "name": "assembled",
					 ...
					 "class": "checkbox",
					 "checked": false,
					 "text": 'Select this option if your input sequence file(s) contain assembled data and include the coverage information within each sequence header as described <a target="blank" href="https://blog.mg-rast.org/mg-rast-v3-2-faq/#assembled_pipeline">here</a>.' } ] },
				   { "name": "dereplication",
				     "fields": [
					 { "id": "dereplication",
					   "name": "dereplication",
